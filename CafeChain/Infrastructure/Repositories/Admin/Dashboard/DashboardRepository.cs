...
public sealed class DashboardRepository : IDashboardRepository
{
    ...
    private static StoreRankingRow MapStoreRanking(DbDataReader r) => new() { StoreId = Int(r,"StoreId"), StoreName = String(r,"StoreName"), TotalOrders = Long(r,"TotalOrders"), NetSales = Decimal(r,"NetSales"), AverageOrderValue = Decimal(r,"AverageOrderValue"), Rank = Int(r,"Rank"), ContributionPercent = Decimal(r,"ContributionPercent"), DataStatus = String(r,"DataStatus") };
    private static PaymentMethodMixRow MapPaymentMethodMix(DbDataReader r) => new() { PaymentMethodId = Int(r,"PaymentMethodId"), PaymentMethodCode = String(r,"PaymentMethodCode"), PaymentMethodName = String(r,"PaymentMethodName"), TotalTransactions = Long(r,"TotalTransactions"), Amount = Decimal(r,"Amount"), Share = Decimal(r,"RevenueShare"), TransactionShare = Decimal(r,"TransactionShare"), RevenueShare = Decimal(r,"RevenueShare"), DataStatus = String(r,"DataStatus") };
    ...
    private static InventoryThresholdRiskRow MapThresholdRisk(DbDataReader r)
    {
        var available = Decimal(r, "AvailableQty");
        var minimum = NullableDecimal(r, "MinStockLevel");
        return new InventoryThresholdRiskRow { StoreInventoryId=Int(r,"StoreInventoryId"),StoreId=Int(r,"StoreId"),IngredientId=Int(r,"IngredientId"),IngredientName=String(r,"IngredientName"),AvailableQty=available,ReservedQty=Decimal(r,"ReservedQty"),MinStockLevel=minimum,MaxNegativeQty=Decimal(r,"MaxNegativeQty"),QuantityAboveMinimum=Decimal(r,"QuantityAboveMinimum"),RiskIngredientCount=available < 0 || minimum.HasValue && available < minimum.Value ? 1 : 0,DataStatus=String(r,"DataStatus") };
    }
    private static InventoryReorderRow MapReorder(DbDataReader r)
    {
        var requested = Decimal(r, "RequestedQuantity");
        var suggested = NullableDecimal(r, "SuggestedQuantity");
        return new InventoryReorderRow { RestockRequestId=Int(r,"RestockRequestId"),StoreId=Int(r,"StoreId"),StoreName=String(r,"StoreName"),IngredientId=Int(r,"IngredientId"),IngredientCode=String(r,"IngredientCode"),IngredientName=String(r,"IngredientName"),Unit=String(r,"Unit"),OnHandQuantity=Decimal(r,"OnHandQuantity"),ReservedQuantity=Decimal(r,"ReservedQuantity"),AvailableQuantity=Decimal(r,"AvailableQuantity"),MinimumStock=NullableDecimal(r,"MinimumStock"),ShortageQuantity=Decimal(r,"ShortageQuantity"),RequestedQuantity=requested,SuggestedQuantity=suggested,EffectiveSuggestedQuantity=suggested ?? requested,SuggestionAverageDailyUsageSnapshot=NullableDecimal(r,"SuggestionAverageDailyUsageSnapshot"),SuggestionLeadTimeDaysSnapshot=NullableInt(r,"SuggestionLeadTimeDaysSnapshot"),SuggestionIncomingQuantitySnapshot=NullableDecimal(r,"SuggestionIncomingQuantitySnapshot"),SuggestionReason=NullableString(r,"SuggestionReason"),Status=String(r,"Status"),Priority=String(r,"Priority"),CreatedAt=Date(r,"CreatedAt"),DataStatus=String(r,"DataStatus") };
    }
    ...
    private static OverduePurchaseOrderRow MapOverduePurchaseOrder(DbDataReader r) => new() { PurchaseOrderId=Int(r,"PurchaseOrderId"),Code=String(r,"Code"),StoreId=Int(r,"StoreId"),StoreName=String(r,"StoreName"),SupplierId=Int(r,"SupplierId"),SupplierName=String(r,"SupplierName"),Status=String(r,"Status"),OrderDate=Date(r,"OrderDate"),ExpectedDeliveryAtUtc=Date(r,"ExpectedDeliveryAtUtc"),OverdueDays=Int(r,"OverdueDays"),OrderedValue=Decimal(r,"OrderedValue"),DataStatus=String(r,"DataStatus") };
    ...
    private static BomHealthRow MapBomHealth(DbDataReader r)
    {
        var recipeCount = Int(r, "RecipeCount");
        var invalidLineCount = Int(r, "InvalidLineCount");
        return new BomHealthRow { DrinkId=Int(r,"DrinkId"),DrinkCode=String(r,"DrinkCode"),DrinkName=String(r,"DrinkName"),RecipeCount=recipeCount,RecipeLineCount=Int(r,"RecipeLineCount"),InvalidLineCount=invalidLineCount,BomIssueCount=recipeCount == 0 ? 1 : invalidLineCount,DataStatus=String(r,"DataStatus") };
    }
    ...
    private static CategoryPerformanceRow MapCategoryPerformance(DbDataReader r) => new() { CategoryId=NullableInt(r,"CategoryId"),CategoryName=String(r,"CategoryName"),TotalSold=Int(r,"TotalSold"),Revenue=Decimal(r,"Revenue"),ConfirmedCogs=Decimal(r,"ConfirmedCogs"),ConfirmedGrossProfit=Decimal(r,"ConfirmedGrossProfit"),ConfirmedMarginRate=Decimal(r,"ConfirmedMarginRate"),ContributionPercent=Decimal(r,"ContributionPercent"),DataStatus=String(r,"DataStatus") };
