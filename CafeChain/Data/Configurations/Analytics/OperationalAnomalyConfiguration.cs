...
public sealed class OperationalAnomalyConfiguration : IEntityTypeConfiguration<OperationalAnomaly>
{
    public void Configure(EntityTypeBuilder<OperationalAnomaly> e)
    {
        ...
        e.Property(x => x.BusinessDate).HasColumnType("date").IsRequired();
        e.Property(x => x.DetectionVersion).HasMaxLength(20).IsRequired();
        ...
        e.Property(x => x.FeedbackNote).HasMaxLength(500);
        ...
        e.HasIndex(x => new { x.StoreId, x.BusinessDate, x.MetricCode, x.DetectionVersion }).IsUnique();
        ...
        e.HasOne(x => x.ResolvedByStaff).WithMany().HasForeignKey(x => x.ResolvedByStaffId).OnDelete(DeleteBehavior.Restrict);
    }
}
