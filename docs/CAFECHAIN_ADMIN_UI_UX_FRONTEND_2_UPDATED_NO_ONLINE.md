...
> **Cập nhật phạm vi sau khi chốt bỏ kênh Online:** Source vẫn có **118 file Admin `.cshtml`** được rà soát. Tuy nhiên, `AdminOrder/History.cshtml`, `AdminVoucher/Index.cshtml` và `AdminWheel/Index.cshtml` là ba view Online cũ đã bị loại khỏi dự án, nên phạm vi thiết kế lại thực tế còn **115 view**. Ba view này không được chỉnh và không được đưa vào prompt hoặc checklist nghiệm thu.
...
- Không nên viết lại DOM hàng loạt của 118 view đã rà soát vì rủi ro phá JavaScript, Razor binding và workflow rất cao; chỉ 115 view còn thuộc phạm vi triển khai sau khi loại kênh Online.
...
### Phạm vi đã loại bỏ — Kênh Online

- Kênh Online đã được bỏ khỏi phạm vi dự án.
- Không thiết kế lại và không chỉnh `AdminOrder/History.cshtml`.
- Không thiết kế lại và không chỉnh `AdminVoucher/Index.cshtml`.
- Không thiết kế lại và không chỉnh `AdminWheel/Index.cshtml`.
- Không đưa Order History Online, Voucher hoặc Wheel vào prompt Antigravity, danh sách file giao việc hay checklist regression.
...
- **57/115 file `.cshtml` trong phạm vi đang triển khai**.
- **19/32 file CSS**, khoảng **17.120 dòng CSS**.
- Nhóm nghiệp vụ: Dashboard Admin, kho–mua hàng, nhận hàng, tồn kho, quản lý đá, nhân sự, phân quyền, lịch ca, cửa hàng, nhà cung cấp, cài đặt và module Category.
- Tổng phạm vi 115 view là số lẻ nên chia gần đều nhất: Frontend 1 nhận 58 view, Frontend 2 nhận 57 view.
...
#### Store/Supplier/System/Category

- Store CRUD đồng bộ; không sửa Leaflet/map JavaScript.
- Category Index/Create/Edit được giao trọn module để cân khối lượng; không chia file Category cho Frontend 1.
- Supplier và SupplierQuality dùng cùng form/table/status contract.
- Settings, Notifications, Profile gọn và rõ.
- Category Index/Create/Edit dùng cùng page header, form control, button, table và status contract của toàn Admin.
...
- [ ] Store map hoạt động như cũ; Category CRUD giữ nguyên binding, validation và submit flow.
...
### A.2. CSS — 19 file, khoảng 17.120 dòng
...
13. KÊNH ONLINE ĐÃ BỎ: KHÔNG CHỈNH AdminOrder/History.cshtml, AdminVoucher/Index.cshtml HOẶC AdminWheel/Index.cshtml.
14. MODULE CATEGORY ĐƯỢC GIAO TRỌN CHO FRONTEND 2; PHẢI GIỮ NGUYÊN BINDING, VALIDATION, ID, NAME, ASP-* VÀ MỌI HOOK HIỆN CÓ.
...
- Đồng bộ Store, Supplier, SupplierQuality, Ingredient, UnitConversion và Category.
- Đồng bộ Profile, Notification, Settings và Category.
...
- Store giữ Leaflet/map hooks; Category giữ nguyên form binding, validation, modal/hook hiện có nếu có.
...
- Frontend 2 chịu trách nhiệm **57/115 view đang triển khai** và **19/32 CSS**, khoảng **17.120 dòng CSS**.
