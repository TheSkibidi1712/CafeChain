...
public sealed class PurchaseOrderBatchSqlServerIssue186Tests : IAsyncLifetime
{
    ...
    public async Task SqlServer_ConcurrentBatchCreate_OneLogicalBatch()
    {
        ...
        Assert.Equal(1, await verify.PurchaseOrders.CountAsync());
        Assert.Equal(1, await verify.PurchaseOrderLineAllocations.CountAsync());
    }
    ...
    public async Task SqlServer_ConcurrentAllocation_DoesNotOverAllocate()
    {
        ...
        Assert.Single(results.Where(x => !x.IsSuccess));
        await using var verify = CreateContext();
        var line = await verify.PurchaseAdviceLines.AsNoTracking().SingleAsync();
        Assert.Equal(5m, line.AllocatedToPoBaseQuantity);
        Assert.Equal(5m, await verify.PurchaseOrderLineAllocations.SumAsync(x => x.AllocatedBaseQuantity));
    }
    ...
    public async Task SqlServer_BatchAndChildPos_AreAtomicAndTraceable()
    {
        ...
        await using var verify = CreateContext();
        var allocation = await verify.PurchaseOrderLineAllocations.AsNoTracking().SingleAsync();
    ...
    private static async Task<Seed> SeedAsync()
    {
        ...
        return new Seed(staff.StaffId, supplier.SupplierId, offer.IngredientSupplierId, line.PurchaseAdviceLineId, Convert.ToBase64String(line.RowVersion));
    }

    private sealed record Seed(int StaffId, int SupplierId, int OfferId, int LineId, string RowVersion);
}
